...
export default async function TechniciansPage() {
  ...
  const { data: technicians } = await supabaseAdmin.from('technicians').select('*, jobs(id, status)').eq('status','active').order('name')
  return (
    <AppLayout user={session}>
      <div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:14}}>
          ...
          {!technicians?.length&&<div style={{gridColumn:'1/-1',textAlign:'center',padding:32,color:'var(--text-muted)'}}>ยังไม่มีช่าง</div>}
        </div>
      </div>
    </AppLayout>
  )
