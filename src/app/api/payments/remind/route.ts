...
export async function POST(req: NextRequest) {
  ...
  if (!customerLineId) {
    ...
    if (customerId) {
      ...
      if (gs?.group_id) {
        ...
        const { token: groupToken } = await getLineOACredentials()
  ...
  const { token: dmToken } = await getLineOACredentials()
  const res = await fetch('https://api.line.me/v2/bot/message/push', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${dmToken}` },
