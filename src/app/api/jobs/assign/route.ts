...
export async function POST(req: NextRequest) {
  ...
  const results: { job_id: string; success: boolean; notified: boolean }[] = []
  ...
  for (const id of ids) {
    ...
    if (error) { results.push({ job_id: id, success: false, notified: false }); continue }

    let notified = false
    if (notify && tech.line_user_id && job) {
    ...
    results.push({ job_id: id, success: true, notified })
  }
