...
export async function POST(req: NextRequest) {
  ...
  // Sync role ไปที่ user_sessions (ถ้า user นี้เคย login ผ่าน LINE แล้ว)
  if (line_user_id) {
    await supabaseAdmin.from('user_sessions')
      .update({ role }).eq('line_user_id', line_user_id)
  }

  // ถ้าเป็น technician → สร้างใน technicians table
  ...
  // ถ้าเป็น customer → สร้างใน customers table
  if (role === 'customer') {
