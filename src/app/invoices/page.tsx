...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<Record<string,string>> }) {
  ...
  let query = supabaseAdmin.from('invoices').select('*, customer:customers(name,phone)').order('created_at',{ascending:false})
  ...
  // ค้นหาชื่อลูกค้าด้วย — filter ฝั่ง client เพราะ join field ใช้ .or() กับ Supabase JS ตรงๆไม่ได้
  const invoices = q
    ? (invoicesRaw || []).filter(inv => {
        const cust = (inv as unknown as { customer: { name: string } }).customer
        return inv.invoice_no?.toLowerCase().includes(q.toLowerCase()) ||
  ...
  const unpaid=invoices.filter(i=>['unpaid','partial','overdue'].includes(i.status)).length
  ...
  return (
    <AppLayout user={session}>
      <div>
        ...
        <SearchBar action="/invoices" placeholder="ค้นหาเลข Invoice หรือชื่อลูกค้า..." defaultValue={q} showDateRange defaultFrom={from} defaultTo={to} />
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                ...
                {!total&&<tr><td colSpan={7} style={{textAlign:'center',padding:24,color:'var(--text-muted)'}}>{q||from||to ? 'ไม่พบ Invoice ที่ตรงกับการค้นหา' : 'ยังไม่มี Invoice'}</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
