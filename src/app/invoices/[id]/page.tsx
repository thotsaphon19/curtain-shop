...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const cust = (inv as unknown as { customer: { name:string; phone:string; address:string; id:string } }).customer
    ...
    <AppLayout user={session}>
      <div style={{ maxWidth:760 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:16 }}>
          ...
          {/* Actions */}
          <InvoiceActions invoiceId={id} invoiceNo={inv.invoice_no} status={inv.status} hasGroup={!!gs} groupHasKhunthong={gs?.khunthong_added||false}/>
        </div>
