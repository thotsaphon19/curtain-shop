...
export default function InvoiceActions({ invoiceId, invoiceNo, status, hasGroup, groupHasKhunthong, hasCustomerLine }: {
  invoiceId:string; invoiceNo:string; status:string; hasGroup:boolean; groupHasKhunthong:boolean; hasCustomerLine:boolean
}) {
  const [loading, setLoading] = useState<string|null>(null)
  const [msg, setMsg] = useState('')

  async function sendDm() {
    setLoading('dm'); setMsg('')
    const res = await fetch('/api/line/invoice-dm', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body:JSON.stringify({invoice_id:invoiceId}),
    })
    const data = await res.json()
    setMsg(data.success ? `✅ ส่ง Invoice ${invoiceNo} ให้ลูกค้าทาง LINE แล้ว` : `❌ ${data.error||'ส่งไม่สำเร็จ'}`)
    setLoading(null)
  }
  ...
  return (
    <div className="card" style={{ padding:20 }}>
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:10 }}>
        {hasCustomerLine && !isPaid && (
          <button onClick={sendDm} disabled={loading==='dm'} className="btn btn-primary" style={{ background:'var(--blue)' }}>
            📩 {loading==='dm'?'กำลังส่ง...':'ส่ง Invoice ให้ลูกค้า (LINE)'}
          </button>
        )}
        {hasGroup && !isPaid && (
