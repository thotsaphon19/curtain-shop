...
export default async function ReportsPage({ searchParams }: { searchParams: Record<string,string> }) {
  ...
  const { data: jobs } = await supabaseAdmin.from('jobs').select('*, customer:customers(name), technician:technicians(name)').gte('scheduled_date',from).lte('scheduled_date',to).order('scheduled_date')
  ...
  return (
    <AppLayout user={session}>
      <div>
        ...
        <form method="GET" style={{display:'flex',gap:10,flexWrap:'wrap',alignItems:'flex-end',marginBottom:20,padding:16,background:'var(--white)',borderRadius:12,boxShadow:'var(--shadow-sm)'}}>
          <div>
        ...
        <div className="card">
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {(jobs as unknown as Job[]||[]).map((job,i)=>(
                  <tr key={job.id}>
                ...
                {!total&&<tr><td colSpan={6} style={{textAlign:'center',padding:24,color:'var(--text-muted)'}}>ไม่มีข้อมูล</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
