...
interface Tech { id: string; name: string; line_user_id?: string }
interface JobData {
...
export default function QueueBoard({ technicians, jobs, slots, schedules, unassigned, weekStart, weekEnd }: Props) {
  ...
  async function assignToSlot(slotId: string, date: string, time: string, techId: string) {
    ...
    if (data.data) {
      ...
      setMsg(`✅ มอบหมาย "${selectedJob.title}" ให้ ${technicians.find(t => t.id === techId)?.name} แล้ว`)
      setSelectedJob(null)
    } else {
  ...
  // Get slots for a specific tech+date
  const getSlotsFor = useCallback((techId: string, date: string) =>
    slotState.filter(s => s.technician_id === techId && s.slot_date === date),
  [slotState])

  const getJobsFor = useCallback((techId: string, date: string) =>
    jobState.filter(j => j.technician_id === techId && j.scheduled_date === date),
  [jobState])
  ...
  return (
    <div>
      ...
      {msg && (
        <div className={`alert ${msg.startsWith('✅') ? 'alert-success' : 'alert-error'} mb-12`} style={{ marginBottom: 12 }}>
          {msg}
        </div>
      )}
              ...
              {unassignedState.length === 0 ? (
                ...
                return (
                  <div key={job.id}
                    ...
                    {job.amount && <div style={{ fontSize: 10, color: 'var(--green)', fontWeight: 600 }}>฿{job.amount.toLocaleString()}</div>}
                  </div>
                )
              })}
            </div>
                ...
                return (
                  <div key={tech.id} style={{ marginBottom: 4 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: `80px repeat(${weekDays.length}, 1fr)`, gap: 2 }}>
                      ...
                      {weekDays.map(day => {
                        const dateStr = fmt(day)
                        const dayJobs = getJobsFor(tech.id, dateStr)
                        const daySlots = getSlotsFor(tech.id, dateStr)
            ...
            <div className="card">
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>
                    {jobState.filter(j => !techFilter || j.technician_id === techFilter).map(job => {
